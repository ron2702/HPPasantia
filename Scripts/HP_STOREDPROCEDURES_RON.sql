...
CREATE OR REPLACE FUNCTION CONTROL_SALIDA_BORRAR(date, varchar(50), integer, bigint) RETURNS integer AS $$
DECLARE
 RESULT integer;

BEGIN
	IF ((SELECT COUNT(*) FROM CONTROL_SALIDA WHERE FECHA_SALIDA = $1 AND HORA_SALIDA = $2 AND FK_REPRESENTANTE = $3 AND FK_ESTUDIANTE = $4) = 1) THEN 

		DELETE FROM CONTROL_SALIDA WHERE ID = (SELECT ID FROM CONTROL_SALIDA WHERE FECHA_SALIDA  = $1 AND HORA_SALIDA = $2 AND FK_REPRESENTANTE = $3 AND FK_ESTUDIANTE = $4);
		RESULT := 200;
