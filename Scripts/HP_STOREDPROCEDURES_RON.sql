...
CREATE OR REPLACE FUNCTION LUGAR_CONSULTAR_ESTADO() RETURNS TABLE (CODIGO integer, NOMBRE varchar(80)) as $$
DECLARE
BEGIN
...
CREATE OR REPLACE FUNCTION LUGAR_CONSULTAR_MUNICIPIO(varchar(80)) RETURNS TABLE (CODIGO integer, NOMBRE varchar(80)) as $$
DECLARE
 FK_ESTADO integer;
BEGIN

	FK_ESTADO := (SELECT L.CODIGO FROM LUGAR L WHERE L.NOMBRE = $1);
	RETURN QUERY
	SELECT LU.CODIGO, LU.NOMBRE FROM LUGAR LU WHERE LU.TIPO = 'Municipio' AND LU.FK_LUGAR = FK_ESTADO;

END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION LUGAR_CONSULTAR_PARROQUIA(varchar(80)) RETURNS TABLE (CODIGO integer, NOMBRE varchar(80)) as $$
DECLARE
 FK_MUNICIPIO integer;
BEGIN

	FK_MUNICIPIO := (SELECT L.CODIGO FROM LUGAR L WHERE L.NOMBRE = $1);
	RETURN QUERY
	SELECT LU.CODIGO, LU.NOMBRE FROM LUGAR LU WHERE LU.TIPO = 'Parroquia' AND LU.FK_LUGAR = FK_MUNICIPIO;
