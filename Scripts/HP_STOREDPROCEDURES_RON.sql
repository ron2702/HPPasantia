...
BEGIN
	IF ((SELECT COUNT(*) FROM MENSUALIDAD_REPRESENTANTE WHERE CODIGO = $1) = 0) THEN 
...
BEGIN
	IF ((SELECT COUNT(*) FROM MENSUALIDAD_REPRESENTANTE WHERE CODIGO = $1) = 1) THEN 
		...
		UPDATE MENSUALIDAD_REPRESENTANTE SET FECHAPAGO = $2, DATOSPAGO = $3, MONTO = $4, BANCO = $5, COODIGOTRANSFERENCIA = $6, FK_REPRESENTANTE = $7, FK_ESTUDIANTE = $8 WHERE CODIGO = $1;
		RESULT := 201;
...
CREATE OR REPLACE FUNCTION MENSUALIDAD_REPRESENTANTE_CONSULTAR_TODOS() RETURNS TABLE (CODIGO_MR varchar(100), FECHAPAGO_MR date, DATOSPAGO_MR varchar(100), MONTO_MR integer, BANCO_MR varchar(30), COODIGOTRANSFERENCIA_MR varchar(50), CEDULA_RE integer, PRIMERNOMBRE_RE varchar(30), PRIMERAPELLIDO_RE varchar(30), CEDULAESCOLAR_EST bigint, PRIMERNOMBRE_EST varchar(30), PRIMERAPELLIDO_EST varchar(30)) as $$
DECLARE

BEGIN

	RETURN QUERY
	SELECT MR.CODIGO, MR.FECHAPAGO, MR.DATOSPAGO, MR.MONTO, MR.BANCO, MR.COODIGOTRANSFERENCIA, RE.CEDULA, RE.PRIMERNOMBRE, RE.PRIMERAPELLIDO, ES.CEDULAESCOLAR, ES.PRIMERNOMBRE, ES.PRIMERAPELLIDO
