...
BEGIN
	...
	IF ((SELECT COUNT(*) FROM CONTROL_SALIDA WHERE FECHA_SALIDA = $1) = 0) THEN 

		FK_REPRESENTANTE := (SELECT CEDULA FROM REPRESENTANTE WHERE CEDULA = $3);
		FK_ESTUDIANTE := (SELECT CEDULAESCOLAR FROM ESTUDIANTE WHERE CEDULAESCOLAR = $4);
		INSERT INTO CONTROL_SALIDA (FECHA_SALIDA, HORA_SALIDA, FK_REPRESENTANTE, FK_ESTUDIANTE) VALUES ($1, $2, FK_REPRESENTANTE, FK_ESTUDIANTE);
		RESULT := 201;
	ELSE
		RESULT := 204;
	END IF;
