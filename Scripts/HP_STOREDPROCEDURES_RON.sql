...
CREATE OR REPLACE FUNCTION CONTROL_SALIDA_CONSULTAR() RETURNS TABLE(FECHA date, HORA varchar(50), CEDULA_REP integer, PRIMERNOMBRE_REP varchar(30), PRIMERAPELLIDO_REP varchar(30), CEDULAESCOLAR_EST bigint, PRIMERNOMBRE_EST varchar(30), PRIMERAPELLIDO_EST varchar(30)) AS $$
DECLARE

BEGIN
	 RETURN QUERY
	 SELECT CS.FECHA_SALIDA, CS.HORA_SALIDA, RE.CEDULA, RE.PRIMERNOMBRE, RE.PRIMERAPELLIDO, EST.CEDULAESCOLAR, EST.PRIMERNOMBRE, EST.PRIMERAPELLIDO
	 FROM CONTROL_SALIDA CS, REPRESENTANTE RE, ESTUDIANTE EST
	 WHERE CS.FK_REPRESENTANTE = RE.CEDULA AND CS.FK_ESTUDIANTE = EST.CEDULAESCOLAR
	 ;
END;
$$ LANGUAGE plpgsql;
